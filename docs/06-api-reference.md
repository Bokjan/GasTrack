...
> **权限模型**：所有记录接口均支持**共享车辆访问**。系统先检查用户是否为车辆所有者，若否则检查车辆是否通过群组共享给用户。非车主用户只能编辑/删除自己创建的记录，车主可操作所有记录。
...
> **权限模型**：车辆级统计接口（6.1/6.3/6.4）支持**共享车辆访问**，群组成员可查看共享给自己的车辆统计数据。全局总览（6.2）仅统计用户自有车辆。
...
> **权限模型**：创建提醒时支持**共享车辆**，用户可为自己拥有或群组内共享给自己的车辆创建保养提醒。
