...
- ✅ 汇率参考（只读展示，不做实时兑换）(P2) — 后端 `ExchangeRateService`（frankfurter.app + 内存缓存 24h TTL），`GET /api/v1/exchange-rates`；前端设置页汇率表、仪表盘/统计页 Tooltip 参考换算、记录详情页多币种 Tag；用户可在设置页选择「参考换算币种」（`reference_currency` 字段），未设置时自动推导（USD↔EUR）
